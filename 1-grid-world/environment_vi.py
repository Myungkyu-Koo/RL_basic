"""
policy_table 없이 다음 state의 value function 값들을 통해 improvement 진행
"""
...
class GraphicDisplay(tk.Tk):
    ...
    def _build_canvas(self):
        canvas = tk.Canvas(self, bg='white',
                           ...
                           width=WIDTH * UNIT)
        # 버튼 초기화
        iteration_button = tk.Button(self, text="Evaluate",
                                     command=self.evaluate_value)
        iteration_button.configure(width=10, activebackground="#33B5E5")
        canvas.create_window(WIDTH * UNIT * 0.13, (HEIGHT * UNIT) + 10,
                             window=iteration_button)

        policy_button = tk.Button(self, text="Improve",
        ...
        policy_button = tk.Button(self, text="Reset", command=self.reset)
        policy_button.configure(width=10, activebackground="#33B5E5")
    ...
    def load_images(self):      # 'img' 폴더 내 map image 로드
    ...
    def reset(self):    # 모든 states, policy 초기화
        if self.is_moving == 0:
            self.iteration_count = 0
            self.improvement_count = 0
            for i in self.texts:
    ...
    def draw_from_values(self, state, action_list):     # value-function 값들을 바탕으로 greedy improvement
        i = state[0]
        j = state[1]
        for action in action_list:
            self.draw_one_arrow(i, j, action)

    def print_value_table(self, values):        # value_table을 기반으로 map 상에 value-function 입력
    ...
    def evaluate_value(self):      # Policy evaluation
    ...
    def improve_policy(self):       # Policy improvement
...
class Env:
    ...
    def get_reward(self, state, action):        # (state, action)의 reward를 계산
        next_state = self.state_after_action(state, action)
        return self.reward[next_state[0]][next_state[1]]        # 다음 state 좌표의 reward 값을 return
    ...
    @staticmethod
    def check_boundary(state):      # Box가 map boundary를 벗어나지 않도록 / state로는 좌표값이 전달
